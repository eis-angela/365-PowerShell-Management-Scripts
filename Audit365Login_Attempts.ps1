...
$operation="userloginfailed" ## use this line to report all logins
...
start-transcript "C:\temp\o365-login-audit.txt" | Out-Null                                        ## Log file created in parent directory that is overwritten on each run
...
if ($csv) {                                                     ## If CSV paramter set
    write-host -foregroundcolor $processmessagecolor "Writing all output to file o365-login-audit$(get-date -f yyyyMMddHHmmss).csv in temp directory"
    $displays | Select-Object LocalTime, ClientIP, Operation, UserId | export-csv -path "C:\temp\o365-login-audit$(get-date -f yyyyMMddHHmmss).csv" -NoTypeInformation ## Export array results to CSV file
}
...
Stop-Transcript | Out-Null          ## End transcript